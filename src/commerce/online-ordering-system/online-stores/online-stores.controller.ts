...
export class OnlineStoresController {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a new online store for the authenticated user\'s merchant. Only portal administrators and merchant administrators can create online stores.'
  })
  @ApiCreatedResponse({
  ...
  @ApiForbiddenResponse({ 
    description: 'Forbidden - You must be associated with a merchant to create online stores',
    type: ErrorResponse,
  })
  @ApiNotFoundResponse({ 
  ...
  @ApiConflictResponse({ 
    description: 'Conflict - An online store with the same subdomain already exists for this merchant',
    type: ErrorResponse,
  })
  @ApiBody({ 
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves a paginated list of online stores for the authenticated user\'s merchant. Supports filtering by subdomain, theme, currency, active status, and creation date.'
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiForbiddenResponse({ 
    description: 'Forbidden - User must be associated with a merchant to view online stores',
    type: ErrorResponse,
  })
  @ApiBadRequestResponse({ 
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves a specific online store by its ID. Users can only access online stores from their own merchant.'
  })
  ...
  @ApiForbiddenResponse({ description: 'Forbidden - You can only view online stores from your own merchant', type: ErrorResponse })
  @ApiNotFoundResponse({ description: 'Online store not found', type: ErrorResponse })
  @ApiBadRequestResponse({ description: 'Invalid online store ID', type: ErrorResponse })
  async findOne(
  ...
  @ApiOperation({ 
    ...
    description: 'Updates an existing online store for the authenticated user\'s merchant. Only portal administrators and merchant administrators can update online stores. All fields are optional.'
  })
  @ApiParam({ 
    name: 'id', 
    type: Number, 
  ...
  @ApiForbiddenResponse({ 
    description: 'Forbidden - You can only update online stores from your own merchant',
    type: ErrorResponse,
  })
  @ApiNotFoundResponse({ 
  ...
  async update(
    ...
    const authenticatedUserMerchantId = req.user?.merchant?.id;
    return this.onlineStoresService.update(id, dto, authenticatedUserMerchantId);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Performs a soft delete by changing the online store status to "deleted". Only merchant administrators can delete online stores from their own merchant.'
  })
  @ApiParam({ name: 'id', type: Number, description: 'Online store ID to delete' })
  @ApiOkResponse({ 
    description: 'Online store soft deleted successfully',
    type: OneOnlineStoreResponseDto
  })
  @ApiUnauthorizedResponse({ description: 'Unauthorized', type: ErrorResponse })
  @ApiForbiddenResponse({ description: 'Forbidden - You can only delete online stores from your own merchant', type: ErrorResponse })
  @ApiNotFoundResponse({ description: 'Online store not found', type: ErrorResponse })
  @ApiBadRequestResponse({ description: 'Invalid online store ID', type: ErrorResponse })
  @ApiConflictResponse({ description: 'Online store is already deleted', type: ErrorResponse })
  async remove(
    @Param('id', ParseIntPipe) id: number,
    @Request() req: any,
  ): Promise<OneOnlineStoreResponseDto> {

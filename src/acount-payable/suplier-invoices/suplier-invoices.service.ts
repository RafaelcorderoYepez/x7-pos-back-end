...
import {
  GetSuplierInvoicesQueryDto,
  SuplierInvoiceSortBy,
} from './dto/get-suplier-invoices-query.dto';
import {
  SuplierInvoiceResponseDto,
  OneSuplierInvoiceResponseDto,
...
export class SuplierInvoicesService {
  ...
  async findAll(
    query: GetSuplierInvoicesQueryDto,
  ): Promise<PaginatedSuplierInvoicesResponseDto> {
    const page = query.page ?? 1;
    const limit = query.limit ?? 10;
    const skip = (page - 1) * limit;
    const sortBy = query.sortBy ?? SuplierInvoiceSortBy.CREATED_AT;
    ...
    const orderColumn =
      sortBy === SuplierInvoiceSortBy.INVOICE_DATE
        ? 'inv.invoice_date'
        : sortBy === SuplierInvoiceSortBy.DUE_DATE
          ? 'inv.due_date'
          : sortBy === SuplierInvoiceSortBy.TOTAL_AMOUNT
            ? 'inv.total_amount'
            : sortBy === SuplierInvoiceSortBy.STATUS
              ? 'inv.status'
              : 'inv.created_at';
  ...
  async findOne(id: number): Promise<OneSuplierInvoiceResponseDto> {
    if (!id || id <= 0) {
  ...
  async remove(id: number): Promise<OneSuplierInvoiceResponseDto> {
    if (!id || id <= 0) {
      throw new BadRequestException('Invalid supplier invoice ID');
    }
